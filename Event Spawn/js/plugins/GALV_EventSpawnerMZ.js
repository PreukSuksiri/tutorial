...
//  2020-10-24 - Modified by MystNix to allow spawn by variable
//-----------------------------------------------------------------------------
 ...
 *      // type    = 'xy' or 'regions' or 'var' depending where you want to spawn event
 *                   'xy' spawns on a specific x,y location
 *                   'regions' spawns on a rangom region in the data specified
*					'var' spawns on a specific location stored in variable v1,v2
 *      // data    = [x,y] if using 'xy' type above
 *                   [id,id,id,id,id] if using 'regions' above
 *                   [v1,v2] where v1 is variable number represented x and v2 represented y
...
Galv.SPAWN.event = function(eventId,type,data,overlap,save) {
	...
	if (type == "regions") {
		...
		// Spawn by variable v1,v2
		const v1 = $gameVariables.value(parseInt(data[0]));
		const v2 = $gameVariables.value(parseInt(data[1]));
		if (Galv.SPAWN.canSpawnOn(v1,v2)) $gameMap.spawnEvent(eventId,v1,v2,save);
	};
};
...
Galv.SPAWN.unspawnbyid = function(eId) {
	if ($gameMap._events[eId].isSpawnEvent) {
		$gameMap.unspawnEvent(eId);
		if (Galv.SPAWN.onScene()) SceneManager._scene._spriteset.unspawnEvent(eId); // eId undefined?
	};
